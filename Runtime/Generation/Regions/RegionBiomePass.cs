...
namespace Jolybob.ProceduralWorld
{
    ...
    /// Position-aware resolvers receive the same deterministic world coordinates used by the chunk generator.
    /// </summary>
            ...
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    RegionId region = positionAwareResolver != null
                        ? positionAwareResolver.Resolve(sample, worldX, worldY)
