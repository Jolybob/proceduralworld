...
namespace Jolybob.ProceduralWorld
{
    ...
    public sealed class WorldPlanValidationIssue
    {
        ...
        public WorldPlanValidationIssue(WorldPlanValidationSeverity severity, string code, string message, string nodeId = null, string connectionId = null)
        {
            Severity = severity;
    ...
    public sealed class WorldPlanCompiler
    {
        public WorldPlanCompilationResult Compile(int seed, WorldPlanGraphDefinition definition)
        {
            ...
            for (int i = 0; i < orderedNodes.Count; i++)
            {
                ...
                WorldPlanNode runtimeNode = new WorldPlanNode(i, definitionNode, typeById[definitionNode.TypeId]);
            ...
            return new WorldPlanCompilationResult(new WorldPlan(seed, runtimeNodes, runtimeConnections), validation);
        }
        ...
        public WorldPlanValidationResult Validate(WorldPlanGraphDefinition definition)
        {
            ...
            for (int i = 0; i < definition.NodeTypes.Count; i++)
            {
                ...
                if (type == null)
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "NullNodeType", "Node type definition is null."));
                    continue;
                }
                if (typeById.ContainsKey(type.Id))
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicateNodeType", "Node type ID is duplicated: " + type.Id));
                    continue;
                }
                ...
                for (int p = 0; p < type.Ports.Count; p++)
                {
                    ...
                    if (port == null)
                    {
                        issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "NullPort", "Node type contains a null port."));
                        continue;
                    }
                    if (!portIds.Add(port.Id))
                        issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicatePort", "Port ID is duplicated on node type '" + type.Id + "': " + port.Id));
                }
            }
            ...
            for (int i = 0; i < definition.Nodes.Count; i++)
            {
                ...
                if (node == null)
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "NullNode", "Node definition is null."));
                    continue;
                }
                if (!nodeById.TryAdd(node.Id, node))
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicateNode", "Node ID is duplicated: " + node.Id, node.Id));
                    continue;
                }
                if (!typeById.TryGetValue(node.TypeId, out WorldPlanNodeTypeDefinition nodeType))
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "UnknownNodeType", "Node '" + node.Id + "' references missing node type '" + node.TypeId + "'.", node.Id));
                    continue;
                }
                ...
                for (int p = 0; p < node.Properties.Count; p++)
                {
                    ...
                    if (property == null)
                    {
                        issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "NullProperty", "Node contains a null property.", node.Id));
                        continue;
                    }
                    if (!propertyKeys.Add(property.Key))
                        issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicateProperty", "Property key is duplicated: " + property.Key, node.Id));
                    if (nodeType.PropertyKeys.Count > 0 && !ContainsOrdinal(nodeType.PropertyKeys, property.Key))
                        issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Warning, "UnknownProperty", "Property '" + property.Key + "' is not declared by node type '" + node.TypeId + "'.", node.Id));
                }
            }
            ...
            for (int i = 0; i < definition.Connections.Count; i++)
            {
                ...
                if (connection == null)
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "NullConnection", "Connection definition is null."));
                    continue;
                }
                if (!connectionIds.Add(connection.Id))
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicateConnection", "Connection ID is duplicated: " + connection.Id, null, connection.Id));

                if (!nodeById.TryGetValue(connection.SourceNodeId, out WorldPlanNodeDefinition sourceNode))
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "MissingSourceNode", "Connection source node does not exist: " + connection.SourceNodeId, null, connection.Id));
                    continue;
                }
                if (!nodeById.TryGetValue(connection.TargetNodeId, out WorldPlanNodeDefinition targetNode))
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "MissingTargetNode", "Connection target node does not exist: " + connection.TargetNodeId, null, connection.Id));
                    continue;
                }

                if (!typeById.TryGetValue(sourceNode.TypeId, out WorldPlanNodeTypeDefinition sourceType))
                    continue;
                if (!typeById.TryGetValue(targetNode.TypeId, out WorldPlanNodeTypeDefinition targetType))
                    continue;
                ...
                if (sourcePort == null)
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "MissingSourcePort", "Source port does not exist: " + connection.SourcePortId, sourceNode.Id, connection.Id));
                    continue;
                }
                if (targetPort == null)
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "MissingTargetPort", "Target port does not exist: " + connection.TargetPortId, targetNode.Id, connection.Id));
                    continue;
                }

                if (!IsSourceDirectionCompatible(sourcePort.Direction) || !IsTargetDirectionCompatible(targetPort.Direction))
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "InvalidPortDirection", "Connection must run from an output/bidirectional port to an input/bidirectional port.", sourceNode.Id, connection.Id));
                ...
                {
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "IncompatiblePortTypes", "Port semantic types do not match: '" + sourcePort.SemanticType + "' -> '" + targetPort.SemanticType + "'.", sourceNode.Id, connection.Id));
                }
                ...
                if (!connectionKeys.Add(connectionKey))
                    issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "DuplicateConnectionEndpoint", "The same source and target ports are connected more than once.", sourceNode.Id, connection.Id));
                ...
                for (int n = 0; n < definition.Nodes.Count; n++)
                {
                    ...
                    for (int p = 0; p < type.Ports.Count; p++)
                    {
                        ...
                        if (port.Required && connectionCount == 0)
                            issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "RequiredPortUnconnected", "Required port '" + port.Id + "' has no connection.", node.Id));
                        if (!port.AllowMultipleConnections && connectionCount > 1)
                            issues.Add(new WorldPlanValidationIssue(WorldPlanValidationSeverity.Error, "PortOverConnected", "Port '" + port.Id + "' does not allow multiple connections.", node.Id));
                    }
                }
            }
        ...
        {
            return direction == WorldPlanPortDirection.Output || direction == WorldPlanPortDirection.Bidirectional;
        }

        private static bool IsTargetDirectionCompatible(WorldPlanPortDirection direction)
        {
            return direction == WorldPlanPortDirection.Input || direction == WorldPlanPortDirection.Bidirectional;
        }
